...
type raftNode struct {
	...
	snapshotterReady chan *snap.Snapshotter // signals when snapshoter is ready

	snapCount uint64
	transport *rafthttp.Transport

	stopc     chan struct{} // signals proposal channel closed
	httpstopc chan struct{} // signals http server to shu
	httpdonec chan struct{} // signals http server shutdown complete
}

func newRaftNode(id int, peers []string, join bool, persister *persister, proposeC <-chan string,
...
func (rc *raftNode) maybeTriggerSnapshot() {
	...
	data, err := rc.persister.getSnapshot()
	if err != nil {
		log.Panic(err)
	}
